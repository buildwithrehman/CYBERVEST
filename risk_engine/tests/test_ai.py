...
def test_ai_authenticated_request_works():
    ...
    assert data["verified_data_source"] == "FAIR Engine (Demonstration Model Scenario)"
...
def test_ai_optimization_budget():
    ...
    data = response.json()
    assert data["verified_data_source"] == "Optimization Engine (Demonstration Model Scenario)"
...
def test_ai_arbitrary_organization_id_ignored():
    ...
    response = client.post("/api/ai/ask", json={
        ...
        "organization_id": "org_hacker_123"
    })
